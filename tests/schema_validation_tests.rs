...
fn test_detection_schema_is_valid() {
    ...
    let schema_json: Value =
        serde_json::from_str(&schema_content).expect("Detection schema must be valid JSON");
    ...
    assert!(
        compiled.is_ok(),
        "Detection schema must compile successfully"
    );
}
...
fn test_prediction_schema_is_valid() {
    ...
    let schema_json: Value =
        serde_json::from_str(&schema_content).expect("Prediction schema must be valid JSON");
    ...
    assert!(
        compiled.is_ok(),
        "Prediction schema must compile successfully"
    );
}
...
fn test_mapping_schema_is_valid() {
    ...
    let schema_json: Value =
        serde_json::from_str(&schema_content).expect("Mapping schema must be valid JSON");
