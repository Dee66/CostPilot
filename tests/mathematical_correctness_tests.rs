...
fn test_single_resource_pr_produces_identical_leaf_and_aggregate_values() {
    ...
    assert!(resource_cost >= 0.0,
        "Single resource cost should be non-negative: {}", resource_cost);
}
...
fn test_aggregates_equal_sum_of_components() {
    ...
    let sum_of_components: f64 = estimates.iter()
        .map(|e| e.monthly_cost)
        .sum();

    // For this test, we validate that all costs are non-negative and sum correctly
    assert!(sum_of_components >= 0.0,
        "Sum of components should be non-negative: {}", sum_of_components);

    // Validate no individual negative costs
    for estimate in &estimates {
        assert!(estimate.monthly_cost >= 0.0,
            "Individual cost should not be negative: {}", estimate.monthly_cost);
    }
}
...
fn test_no_negative_costs_unless_explicitly_justified() {
    ...
    for estimate in &estimates {
        assert!(estimate.monthly_cost >= 0.0,
            ...
            estimate.resource_id, estimate.monthly_cost);
    }
}
...
fn test_percentages_normalize_to_exactly_100_percent() {
    ...
    let total_percentage: f64 = explanation.final_estimate.components
        .iter()
        .map(|c| c.percentage)
        .sum();
    ...
    assert!((total_percentage - 100.0).abs() < 0.001,
        ...
        total_percentage,
        explanation.final_estimate.components.iter().map(|c| format!("{}: {:.1}%", c.name, c.percentage)).collect::<Vec<_>>());
}
...
fn test_zero_cost_resources_handled_explicitly() {
    ...
    assert_eq!(estimates.len(), 1, "Zero-cost resource should still be included in estimates");
    ...
    assert!(!explanation.final_estimate.components.is_empty(),
