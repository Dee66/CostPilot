...
use costpilot::engines::autofix::drift_safety::{
    CriticalDriftConfig, CriticalDriftDetector, CriticalDriftReason, DriftChecksum, DriftSeverity,
    DriftedAttribute,
};
...
fn test_execution_blocked_by_protected_production_resource() {
    ...
    assert_eq!(
        result.critical_drifts[0].reason,
        CriticalDriftReason::ProtectedEnvironment
    );
}
...
fn test_summary_format_for_passing_check() {
    let detector = CriticalDriftDetector::new();
    let drifts = vec![create_drift("i-0123456789abcdef0", "aws_instance", vec![])];
...
fn test_kms_key_change_detected() {
    ...
    let drifts = vec![create_drift(
        ...
        vec![(
            ...
            "arn:aws:kms:us-east-1:111111111111:key/key2",
            DriftSeverity::Critical,
        )],
    )];
