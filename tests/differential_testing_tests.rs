...
fn test_metamorphic_property_preservation() {
    ...
    assert_eq!(base_detections.len(), modified_detections.len(),
    ...
    if !base_predictions.is_empty() && !modified_predictions.is_empty() {
        ...
        if !base_predictions2.is_empty() {
            assert!((base_predictions[0].monthly_cost - base_predictions2[0].monthly_cost).abs() < 0.01,
...
fn test_deterministic_output_consistency() {
    ...
    for result in &results[1..] {
        assert_eq!(first.0, result.0, "Detection results should be deterministic");
        assert_eq!(first.1, result.1, "Prediction results should be deterministic");
    }
}
...
fn test_oracle_based_validation() {
    ...
    if let Some(prediction) = predictions.first() {
        ...
        if prediction.prediction_interval_low > 0.0 {
            assert!(prediction.prediction_interval_low <= prediction.monthly_cost,
                    "Lower bound should not exceed estimate");
        }
        if prediction.prediction_interval_high > 0.0 {
            assert!(prediction.prediction_interval_high >= prediction.monthly_cost,
...
fn test_input_transformation_invariance() {
    ...
    if !predictions1.is_empty() && !predictions2.is_empty() {
        assert!((predictions1[0].monthly_cost - predictions2[0].monthly_cost).abs() < 0.01,
...
fn test_regression_detection_via_comparison() {
    ...
    // Store baseline
    let baseline_cost = predictions.first()
        .map(|p| p.monthly_cost)
        .unwrap_or(0.0);

    // Test that costs are reasonable (not negative, not extremely high)
    assert!(baseline_cost >= 0.0, "Should have non-negative baseline cost");
    ...
    let cost2 = predictions2.first()
        .map(|p| p.monthly_cost)
        .unwrap_or(0.0);

    assert!((baseline_cost - cost2).abs() < 0.01,
