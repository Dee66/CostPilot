...
#[cfg(test)]
mod tests {
    use crate::wasm::{SandboxLimits, MemoryTracker, validate_input_size};
    ...
    fn test_memory_limit_validation() {
        ...
        assert!(matches!(result, crate::wasm::ValidationResult::Ok));
        ...
        assert!(matches!(
            result,
            crate::wasm::ValidationResult::ExceedsFileSize { .. }
        ));
    }
    ...
    fn test_small_plan_memory() {
        ...
        let resources = parser.parse(&plan).expect("Should parse small plan");
    ...
    fn test_medium_plan_memory() {
        ...
        let resources = parser.parse(&plan).expect("Should parse medium plan");
    ...
    fn test_large_plan_memory() {
        ...
        let parser = PlanParser::new();
        let resources = parser.parse(&plan).expect("Should parse large plan");
    ...
    fn test_memory_pressure() {
        ...
        for iteration in 1..=10 {
            ...
            let parser = PlanParser::new();
            let resources = parser.parse(&plan).unwrap();
    ...
    fn test_nested_json_depth() {
        use crate::wasm::validate_json_depth;
    ...
    fn test_allocation_patterns() {
        ...
        let small_plans: Vec<String> = (0..1000).map(|i| generate_large_plan(1)).collect();
        assert_eq!(small_plans.len(), 1000);
        
        // Pattern 2: Few large allocations
        let large_plans: Vec<String> = (0..10).map(|i| generate_large_plan(100)).collect();
    ...
    fn test_memory_cleanup() {
        ...
        
        {
            ...
            // plan, parser, and resources dropped here
        }
        ...
        let parser = PlanParser::new();
        let resources = parser.parse(&new_plan).unwrap();
        assert_eq!(resources.len(), 5000);
    }
}
