use costpilot::edition::{
    require_premium, Capabilities, EditionContext, EditionMode, UpgradeRequired,
};
...
mod authorization_security_tests {
    ...
    fn test_privilege_escalation_prevention_free_to_premium() {
        ...
        for (feature_name, capability_field) in premium_features {
            ...
            if let Err(e) = result {
                ...
                assert!(
                    upgrade_error.is_some(),
                    "Should get UpgradeRequired error for {}",
                    feature_name
                );
            ...
            let caps = free_edition.capabilities.clone();
            match capability_field {
                "allow_autofix" => {
                    assert!(!caps.allow_autofix, "Free edition should not allow autofix")
                }
                "allow_trend" => assert!(!caps.allow_trend, "Free edition should not allow trend"),
                "allow_mapping_deep" => assert!(
                    !caps.allow_mapping_deep,
                    "Free edition should not allow deep mapping"
                ),
                "allow_slo_enforce" => assert!(
                    !caps.allow_slo_enforce,
                    "Free edition should not allow SLO enforcement"
                ),
                "allow_policy_enforce" => assert!(
                    !caps.allow_policy_enforce,
                    "Free edition should not allow policy enforcement"
                ),
                "allow_explain_full" => assert!(
                    !caps.allow_explain_full,
                    "Free edition should not allow advanced explain"
                ),
    ...
    fn test_unauthorized_access_attempts_blocked() {
        ...
        assert!(
            !caps.allow_predict,
            "Free edition should not allow prediction"
        );
        assert!(
            !caps.allow_explain_full,
            "Free edition should not allow full explain"
        );
        assert!(!caps.allow_autofix, "Free edition should not allow autofix");
        assert!(
            !caps.allow_mapping_deep,
        ...
        assert!(
            !caps.allow_predict
                ...
                && !caps.allow_policy_enforce
                && !caps.allow_slo_enforce,
            "All capabilities should be false for free edition"
        );
    }
    ...
    fn test_role_conflicts_prevented() {
        ...
        let caps = free_edition.capabilities.clone();
        let _premium_caps = Capabilities {
            allow_predict: true,
            allow_autofix: true,
            allow_trend: true,
        ...
        assert!(
            !caps.allow_autofix,
            "Free edition should conflict with premium autofix capability"
        );
        assert!(
            !caps.allow_trend,
            "Free edition should conflict with premium trend capability"
        );
        assert!(
            !caps.allow_mapping_deep,
            "Free edition should conflict with premium deep mapping capability"
        );
        assert!(
            !caps.allow_slo_enforce,
            "Free edition should conflict with premium SLO enforcement capability"
        );
        assert!(
            !caps.allow_policy_enforce,
            "Free edition should conflict with premium policy enforcement capability"
        );
        assert!(
            !caps.allow_explain_full,
            "Free edition should conflict with premium advanced explain capability"
        );
        ...
        for feature in test_features {
            ...
            assert!(
                result.is_err(),
                "Role conflict: {} should be blocked in free edition",
                feature
            );
        }
    }
    ...
    fn test_permission_inheritance_consistency() {
        ...
        assert_eq!(
            free_edition.mode,
            EditionMode::Free,
            "Edition mode should be Free"
        );
        assert!(
            !free_edition.is_premium(),
            "Free edition should not be premium"
        );

        // Test that free edition inheritance blocks all premium features
        assert!(
            !context_caps.allow_autofix
                && !context_caps.allow_trend
                && !context_caps.allow_mapping_deep
                && !context_caps.allow_slo_enforce
                && !context_caps.allow_policy_enforce
                && !context_caps.allow_explain_full,
    ...
    fn test_access_control_list_enforcement() {
        ...
        for (feature, should_be_allowed) in expected_acl {
            let is_allowed = match feature {
                "autofix" => caps.allow_autofix,
                "trend" => caps.allow_trend,
                "deep_mapping" => caps.allow_mapping_deep,
                "slo_enforcement" => caps.allow_slo_enforce,
                "policy_enforcement" => caps.allow_policy_enforce,
                "advanced_explain" => caps.allow_explain_full,
        ...
        let acl_violations = vec![
            ("Autofix", caps.allow_autofix),
            ("Trend Analysis", caps.allow_trend),
            ("Deep Mapping", caps.allow_mapping_deep),
            ("SLO Enforcement", caps.allow_slo_enforce),
            ("Policy Enforcement", caps.allow_policy_enforce),
            ("Advanced Explain", caps.allow_explain_full),
        ];

        for (feature_name, is_allowed) in acl_violations {
            assert!(
                !is_allowed,
                "ACL should prevent {} access in free edition",
                feature_name
            );
            ...
            assert!(
                result.is_err(),
                "ACL should block {} feature access",
                feature_name
            );
        }
    }
    ...
    fn test_capability_boundary_enforcement() {
        ...
        assert_eq!(
            premium_capability_count, 0,
            "Free edition should have zero premium capabilities, found {}",
            premium_capability_count
        );
        ...
        for (feature, category) in boundary_tests {
            ...
            assert!(
                result.is_err(),
                "Capability boundary should prevent {} ({}) access",
                feature,
                category
            );
            ...
            if let Err(e) = result {
                assert!(
                    e.downcast_ref::<UpgradeRequired>().is_some(),
                    "Boundary violation should return UpgradeRequired error for {}",
                    feature
                );
            }
        }
    ...
    fn test_authorization_state_consistency() {
        ...
        for (i, feature) in test_features.iter().enumerate() {
            ...
            assert!(
                free_edition.is_free(),
                "Authorization state should remain free (check {})",
                i
            );
            assert!(
                !free_edition.is_premium(),
                "Authorization state should not become premium (check {})",
                i
            );
            ...
            assert!(
                !caps.allow_autofix,
                "Capabilities should remain restricted (check {})",
                i
            );
            ...
            assert!(
                result.is_err(),
                "Authorization should consistently fail (check {})",
                i
            );
        }

        // Test that state doesn't change after multiple operations
        let final_caps = free_edition.capabilities.clone();
        assert!(
            !final_caps.allow_autofix,
            "Final state should remain restricted"
        );
        assert!(
            !final_caps.allow_trend,
            "Final state should remain restricted"
        );
    }
    ...
    fn test_cross_feature_authorization_isolation() {
        ...
        for feature in features {
            ...
            assert!(
                result.is_err(),
                "Each feature should be independently blocked: {}",
                feature
            );
            ...
            assert!(
                other_result.is_err(),
                "Cross-feature isolation failed for: {}",
                feature
            );
        }

        // Test that capability checks are isolated
        let caps1 = free_edition.capabilities.clone().clone();
        let caps2 = free_edition.capabilities.clone().clone();

        assert_eq!(
            caps1.allow_autofix, caps2.allow_autofix,
