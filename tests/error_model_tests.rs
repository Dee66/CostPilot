...
use costpilot::engines::shared::error_model::{map_category_to_id, CostPilotError, ErrorCategory};
...
fn test_error_ids_follow_naming_convention() {
    ...
    for error in errors {
        assert!(
            error.id.starts_with("E_"),
            "Error ID should start with E_: {}",
            error.id
        );
        assert!(
            error.id.chars().all(|c| c.is_uppercase() || c == '_'),
            "Error ID should be uppercase with underscores: {}",
            error.id
        );
    }
}
...
fn test_error_category_mapping_is_stable() {
    ...
    for category in categories {
        ...
        assert!(
            id.starts_with("E_"),
            "Category mapping should produce E_ prefix"
        );
...
fn test_all_categories_have_mappings() {
    ...
    for category in categories {
        ...
        assert!(
            !id.is_empty(),
            "Category {:?} should have a non-empty ID mapping",
            category
        );
    }
}
...
fn test_remediation_hint_generation() {
    ...
    for error in errors {
        ...
        assert!(
            !hint.is_empty(),
            "Every error should generate a non-empty hint"
        );
...
fn test_remediation_hints_are_actionable() {
    ...
    assert!(
        actionable_verbs
            .iter()
            .any(|verb| hint_lower.contains(verb)),
        "Hint should contain actionable verbs: {}",
        hint
    );
}
...
fn test_error_with_explicit_hint() {
    let error = CostPilotError::parse_error("test").with_hint("Custom remediation hint");
...
fn test_error_with_context() {
    ...
    let error = CostPilotError::parse_error("syntax error").with_context(context.clone());
...
fn test_machine_format_includes_hint() {
    let error = CostPilotError::parse_error("test").with_hint("fix it");
...
fn test_machine_format_includes_context() {
    ...
    let error = CostPilotError::parse_error("test").with_context(context.clone());
...
fn test_error_display_format() {
    let error = CostPilotError::parse_error("syntax error").with_hint("check JSON format");
