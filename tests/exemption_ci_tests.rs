...
use costpilot::engines::policy::{
    check_exemptions_for_ci, ExemptionsFile, PolicyExemption,
    EXIT_SUCCESS, EXIT_EXEMPTION_EXPIRED, EXIT_VALIDATION_ERROR,
};
...
fn test_expired_on_exact_date() {
    ...
    let exemptions_file = ExemptionsFile {
        version: "1.0".to_string(),
        exemptions: vec![
            create_test_exemption("EXE-001", &today_str),
        ],
        metadata: None,
    };
