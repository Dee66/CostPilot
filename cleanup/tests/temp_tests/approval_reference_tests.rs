...
use costpilot::engines::policy::approval_workflow::{ApprovalWorkflowManager, WorkflowError};
...
fn create_test_manager() -> ApprovalWorkflowManager {
    let config = ApprovalConfig {
        min_approvals: 1,
        required_roles: vec![],
        allowed_approvers: vec!["alice@example.com".to_string()],
...
fn test_approval_reference_required_for_flagged_policies() {
    ...
    let mut manager = create_test_manager();
    manager
        .register_policy("flagged-policy".to_string(), None)
        .unwrap();
    manager
        .submit_for_approval("flagged-policy", "author@example.com".to_string())
        .unwrap();
