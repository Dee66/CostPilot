...
use crate::validation::{validate_file, ValidationReport};
...
pub fn execute_batch(
    ...
    match format.as_str() {
        ...
        "text" => {
            ...
            println!("  ❌ Invalid: {}", (total_count - valid_count).to_string().red());
            println!("  🔴 Total errors: {}", error_count.to_string().red());
            println!("  🟡 Total warnings: {}", warning_count.to_string().yellow());
        }
        _ => {
            return Err(format!("Unknown format: {}", format).into());
        }
