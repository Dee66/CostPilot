...
use crate::engines::performance::{
    PerformanceMonitor, PerformanceBudgets, PerformanceReport,
};
...
pub fn execute_performance_command(cmd: PerformanceCommand) -> Result<String, String> {
    match cmd {
        ...
        PerformanceCommand::CheckRegressions { report_file } => execute_check_regressions(&report_file),
...
fn execute_budgets() -> Result<String, String> {
    ...
    output.push_str(&format!("WASM Sandbox:\n"));
    output.push_str(&format!("  Max Memory: {}MB\n", budgets.wasm.max_memory_mb));
    output.push_str(&format!("  Max Execution: {}ms\n", budgets.wasm.max_execution_ms));
    output.push_str(&format!("  Max Stack Depth: {}\n", budgets.wasm.max_stack_depth));
    output.push_str(&format!("  Max Bytecode: {}MB\n\n", budgets.wasm.max_bytecode_mb));
    
    output.push_str(&format!("Circuit Breaker:\n"));
...
fn execute_set_baseline(from_file: Option<PathBuf>) -> Result<String, String> {
    ...
    if let Some(file) = from_file {
        ...
        Ok(format!("✅ Performance baseline set from: {}", file.display()))
    } else {
...
fn execute_history(engine: Option<String>, limit: Option<usize>) -> Result<String, String> {
    let history_path = get_history_path()?;
    let monitor = PerformanceMonitor::load(&history_path)?;
...
fn get_history_path() -> Result<PathBuf, String> {
    ...
    Ok(PathBuf::from(home).join(".costpilot").join("performance.json"))
}
