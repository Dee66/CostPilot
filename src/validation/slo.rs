...
use crate::engines::slo::slo_types::{Slo, EnforcementLevel};
use std::path::Path;
...
impl SloValidator {
    ...
    fn validate_slos(
        slos: &std::collections::HashMap<String, Slo>,
        ...
        for (slo_name, slo) in slos {
            ...
            if slo.target.is_empty() {
                report.add_error(
                    ValidationError::new("SLO has empty target")
                        ...
                        .with_hint("Specify target entity (module name, service name, or 'global')")
                );
            }
            ...
            if slo.threshold.max_value <= 0.0 {
                report.add_error(
                    ValidationError::new(format!("Invalid max threshold: {}", slo.threshold.max_value))
            ...
            if slo.threshold.max_value > 10_000_000.0 {
                report.add_warning(
                    ValidationWarning::new(format!("Very high threshold: ${:.2}", slo.threshold.max_value))
                        .with_field(&format!("{}.threshold.max_value", prefix))
            ...
            if slo.threshold.warning_threshold_percent <= 0.0 || slo.threshold.warning_threshold_percent > 100.0 {
                report.add_error(
                    ValidationError::new(format!("Invalid warning threshold: {}%", slo.threshold.warning_threshold_percent))
            ...
            match slo.enforcement {
                ...
                EnforcementLevel::Block | EnforcementLevel::StrictBlock => {
                    report.add_warning(
                        ValidationWarning::new(format!("SLO uses {:?} enforcement", slo.enforcement))
                            .with_field(&format!("{}.enforcement", prefix))
                            .with_warning_code("W405")
                            .with_suggestion("Ensure blocking is intentional; it will prevent deployments")
                    );
                }
            }
        }
...
"#;
        ...
        let report = SloValidator::validate_file(file.path()).unwrap();
        assert!(!report.is_valid);
        assert!(report.errors.iter().any(|e| e.error_code == Some("E401".to_string())));
    }
...
"#;
        ...
        let report = SloValidator::validate_file(file.path()).unwrap();
        assert!(!report.is_valid);
        assert!(report.errors.iter().any(|e| e.error_code == Some("E402".to_string())));
    }
...
"#;
        ...
        let report = SloValidator::validate_file(file.path()).unwrap();
        assert!(!report.is_valid);
        assert!(report.errors.iter().any(|e| e.error_code == Some("E407".to_string())));
    }
}
