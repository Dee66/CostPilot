...
impl PolicyVersionManager {
    ...
    pub fn increment_version(&self, current_content: &str, changelog: Option<String>) -> Result<PolicyVersion, std::io::Error> {
    ...
    pub fn require_increment_on_change(&self, current_content: &str) -> Result<Option<String>, std::io::Error> {
        if self.has_changed(current_content)? {
            let current_version = self.load_version()
                .map(|v| v.version)
...
mod tests {
    ...
    fn test_increment_semver() {
        ...
        assert_eq!(PolicyVersionManager::increment_semver("2.10.99"), "2.10.100");
    ...
    fn test_version_lifecycle() {
        ...
        let version1 = manager.increment_version(content1, Some("Initial version".to_string())).unwrap();
        
        assert_eq!(version1.version, "1.0.0");
        assert_eq!(version1.content_hash, PolicyVersionManager::calculate_hash(content1));
        ...
        let version2 = manager.increment_version(content2, Some("Updated rules".to_string())).unwrap();
        
        assert_eq!(version2.version, "1.0.1");
        assert_eq!(version2.content_hash, PolicyVersionManager::calculate_hash(content2));
    ...
    fn test_require_increment_on_change() {
        ...
        assert!(result.unwrap().contains("Version must be incremented from 1.0.0"));
    }
