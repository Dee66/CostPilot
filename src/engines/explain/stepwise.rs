...
impl ReasoningChainBuilder {
    ...
    pub fn add_resource_identification(
        &mut self,
        resource_id: &str,
        resource_type: &str,
    ) -> &mut Self {
        ...
        self.chain.add_step(ReasoningStep {
            ...
            description: format!(
                "Detected {} resource requiring cost analysis",
                resource_type
            ),
            input_values: vec![
                InputValue {
    ...
    pub fn add_configuration_extraction(
        &mut self,
        config_key: &str,
        config_value: &str,
        from_plan: bool,
        ...
        let source = if from_plan {
            ValueSource::TerraformPlan
        } else {
        ...
        self.chain.add_step(ReasoningStep {
            ...
            assumptions: if from_plan {
                ...
                vec![format!(
                    "{} not specified, using AWS default: {}",
    ...
    pub fn add_heuristic_lookup(
        ...
        self.chain.add_step(ReasoningStep {
            ...
            description: format!(
                "Retrieved pricing from cost heuristics database (v{})",
                heuristics_version
            ),
            input_values: vec![InputValue {
            ...
            assumptions: vec![
                format!(
                    "Pricing from heuristics v{} (last updated from AWS pricing API)",
                    heuristics_version
                ),
    ...
    pub fn add_cold_start_inference(
        ...
        self.chain.add_step(ReasoningStep {
            ...
            assumptions: vec![format!(
                "{} not found in heuristics, using inference model",
                what
            )],
        });

        self.chain.add_assumption(format!(
            "{} inferred using cold-start logic: {}",
            what, reasoning
        ));
    ...
    pub fn add_interval_estimation(&mut self, range_factor: f64, low: f64, high: f64) -> &mut Self {
        ...
        self.chain.add_step(ReasoningStep {
            ...
            description: format!(
                "Apply ±{:.0}% range to account for uncertainty",
                range_factor * 100.0
            ),
            input_values: vec![InputValue {
            ...
            confidence_impact: None,
            assumptions: vec![format!(
                "Prediction interval accounts for ±{:.0}% variance",
                range_factor * 100.0
            )],
        });
...
mod tests {
    ...
    fn test_reasoning_chain_builder() {
        let mut builder =
            ReasoningChainBuilder::new("aws_instance.web".to_string(), "aws_instance".to_string());
        ...
        builder
            ...
            .add_confidence_scoring(
                0.85,
                vec!["Known resource".to_string(), "Explicit config".to_string()],
            )
