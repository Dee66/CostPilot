...
impl DriftDetector {
    ...
    fn filter_config(&self, config: &HashMap<String, serde_json::Value>) -> HashMap<String, serde_json::Value> {
        config
            .iter()
    ...
    fn determine_severity(
        ...
        if key.contains("encryption") || key.contains("public") || key.contains("vpc") 
            || key.contains("subnet") || key.contains("instance_type") {
...
mod tests {
    ...
    fn test_severity_determination() {
        ...
        // Critical
        let severity = detector.determine_severity("security_group", &json!("sg-1"), &json!("sg-2"));
        assert_eq!(severity, DriftSeverity::Critical);
        
        // High
        let severity = detector.determine_severity("encryption_enabled", &json!(true), &json!(false));
