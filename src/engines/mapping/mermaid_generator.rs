...
impl MermaidGenerator {
    ...
    fn generate_edge_definition(&self, edge: &GraphEdge, graph: &DependencyGraph) -> Result<String, CostPilotError> {
    ...
    fn group_nodes_by_module<'a>(
        ...
        let mut groups: std::collections::HashMap<Option<String>, Vec<&GraphNode>> = std::collections::HashMap::new();
...
"#,
            ...
            graph.metadata.cycles.len(),
            graph.metadata.total_cost.map(|c| format!("{:.2}/mo", c)).unwrap_or_else(|| "N/A".to_string()),
            mermaid_code
        );
...
mod tests {
    ...
    fn test_edge_types() {
        ...
        graph.add_edge(GraphEdge::new("a".to_string(), "b".to_string(), EdgeType::DataFlow));
