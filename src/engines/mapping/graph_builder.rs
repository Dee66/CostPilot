...
impl GraphBuilder {
    ...
    fn create_node_from_resource(&self, change: &ResourceChange) -> Result<GraphNode, CostPilotError> {
        let node_id = self.generate_stable_id(&change.resource_id);
        let label = self.generate_label(change);
        
        let mut node = GraphNode::new_resource(
            node_id,
                ...
                if let Some(to_id) = self.find_resource_by_reference(vpc_id, all_changes) {
                    edges.push(
                        GraphEdge::new(from_id.to_string(), to_id, EdgeType::DependsOn)
                            .with_cost_impact("VPC networking may incur data transfer costs".to_string())
                    );
                }
            }
        }
                ...
                for (key, value) in vars {
                    if key.contains("BUCKET") || key.contains("S3") {
                        if let Some(bucket_ref) = value.as_str() {
                            if let Some(to_id) = self.find_resource_by_reference(bucket_ref, all_changes) {
                                edges.push(
                                    GraphEdge::new(from_id.to_string(), to_id, EdgeType::DataFlow)
                                        .with_cost_impact("S3 requests and data transfer".to_string())
                                );
                            }
                        }
                    }
    ...
    fn generate_label(&self, change: &ResourceChange) -> String {
        ...
        if let Some(last) = parts.last() {
            return last.trim_matches(|c: char| c == '[' || c == ']' || c == '"').to_string();
        }
